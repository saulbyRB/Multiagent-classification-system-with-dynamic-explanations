...
class LogisticRegression(SklearnModel):
    ...
    def adjust_from_feedback(self, signals, X_train, y_train, X_test, y_test):
        ...
        if signals.get("strategy") == "keep":
            print("[LogReg] keep → sin cambios")
            return

        # Ajuste suave y muy estable
        ...
        if signals["strategy"] == "soft_adjust":
            factor = 1 + (factor - 1) / 2

        # C ↑ menos regularización, C ↓ más regularización
